...
TOKEN = '7431767090:AAFDtKKL9HYC4mrWKcMMtg5ilcKno1-2_Ug'
bot = telebot.TeleBot(TOKEN, threaded=False)

# 🛡️ List of authorized user IDs (replace with actual IDs)
AUTHORIZED_USERS = [5468359688,1207069835]
...
def start(message):
    ...
    if user_id not in AUTHORIZED_USERS:
        ...
        welcome_message = (
            ...
            "📞 Contact the owner on Instagram and Telegram:\n"
...
def attack(message):
    ...
    if user_id not in AUTHORIZED_USERS:
        bot.send_message(message.chat.id, "🚫 Access Denied! Contact the owner for assistance: @AkashChutiyaHe")
        return
...
def show_id(message):
    ...
    bot_owner = "abhishekpawara"  # 👑 The bot owner's username  ⋆｡ﾟ☁︎｡⋆｡ ﾟ☾ ﾟ｡⋆⋆｡ﾟ☁︎｡⋆｡ ﾟ☾ ﾟ｡⋆
...
def owner(message):
    ...
    bot.send_message(message.chat.id, "📞 Contact the owner: @abhishekpawara")
